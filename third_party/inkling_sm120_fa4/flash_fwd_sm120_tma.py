...
class FlashAttentionForwardSm120Tma(FlashAttentionForwardBase):
    ...
    def __init__(
        ...
        # PackGQA on the inherited Sm80 epilogue (flash_fwd.py:503) calls
        # pack_gqa.store_O -> compute_ptr (pack_gqa.py:139), which raises crd2idx
        # against the un-packed ((qhead_per_kvhead, seqlen_q), headdim) layout when
        # the kernel is dispatched directly without the pack_gqa_layout transforms
        # that Sm90/Sm100 apply. Disabling pack_gqa here forces the non-packed
        # epilogue branch which works correctly on every GQA/MQA shape we tested.
        # Tracked upstream as Dao-AILab/flash-attention#2484.
        self.pack_gqa = False
    ...
    def __call__(
        ...
        # The TMA kernel does not implement dropout; interface dispatch
        # already gates `use_tma_sm120` on `dropout_p == 0.0`, so we should
        # never see real seeds here. Accepting them as None keeps the
        # call signature compatible with the non-TMA kernels.
