#!/bin/bash
# Same-session DSpark block 5/6/7 sweep on the native SM121 FA4 FP4 stack.
set -euo pipefail
...
RESULT_DIR=${RESULT_DIR:-artifacts/e7-fa4-fp4-block-sweep}
...
start_arm() {
  local block=$1 label="e7-fa4-fp4-block-$1"
  stop_arm
  ssh -f -o BatchMode=yes "$WORKER_SSH" \
    "mkdir -p $(printf '%q' "$WORKER_REPO/$RESULT_DIR") && cd $(printf '%q' "$WORKER_REPO") && exec env MASTER_IP=$(printf '%q' "$MASTER_IP") IF=$(printf '%q' "$IF") HCA=$(printf '%q' "$HCA") GID=$(printf '%q' "$GID") MODELS=$(printf '%q' "$MODELS") IMAGE=$(printf '%q' "$IMAGE") LOG=$(printf '%q' "$WORKER_REPO/$RESULT_DIR/$label-worker.log") ATTN=fa4 MOE=marlin FP4GEMM=flashinfer_trtllm MEMFRAC=0.85 CTX=65536 SPEC=1 BLOCK=$(printf '%q' "$block") GRAPHS=1 GRAPH_BS=$(printf '%q' '1 2 3 4 5 6 7 8 10 12 14 16') RAGGED= INKLING_SHEARED_BIAS=0 MAXREQ=16 PAGE=128 CONTINUOUS_DECODE_STEPS=2 EXTRA_ARGS=$(printf '%q' '--kv-cache-dtype fp4_mx_block16') ./scripts/inkling-sglang-launch.sh 1 </dev/null >/dev/null 2>&1"
  sleep 3
  ...
  grep -q 'DSpark draft greedy proposal folded into the draft cuda graph' \
    "$RESULT_DIR/$label-head.log"
  python3 - "$RESULT_DIR/$label-head.log" "$MIN_FULL_TOKENS" "$block" <<'PY' \
...
if full < minimum:
    raise SystemExit(f"block={block} capacity={full} minimum={minimum}")
print(
    f"block={block} FP4 CAPACITY PASS full_layer_tokens={full} "
    f"headroom={full - minimum} swa_layer_tokens={swa}"
)
PY
  python3 - "$IMAGE" "$block" "$RESULT_DIR/$label-head-inspect.json" \
...
for path_text in sys.argv[3:]:
    ...
    if "SGLANG_OPT_USE_INKLING_SHEARED_BIAS=0" not in env:
        raise SystemExit(f"{path}: score-mod bias contract failed")
print(f"block={block} runtime contract PASS")
PY
  ...
  INKLING_URL=http://127.0.0.1:30000 python3 "$REPO_DIR/benchmarks/chat_bench.py" \
    ...
    --block-size "$block" --require-histogram \
...
python3 "$REPO_DIR/benchmarks/select_block_size.py" \
  "$RESULT_DIR/e7-fa4-fp4-block-5.json" \
  "$RESULT_DIR/e7-fa4-fp4-block-6.json" \
  "$RESULT_DIR/e7-fa4-fp4-block-7.json" \
  --task open-ended --output "$RESULT_DIR/decision.json" || decision_rc=$?
if [ "$decision_rc" -ne 0 ] && [ "$decision_rc" -ne 3 ]; then
  exit "$decision_rc"
fi
echo "PASS: FA4 FP4 DSpark block sweep completed with exact n=32 and T4 brackets" \
