...
case "$ATTN" in
  fa4|triton) ;;
  *) echo "ATTN must be fa4 or triton" >&2; exit 2 ;;
esac
case "$PAGE" in
  1|128) ;;
...
for path_text in sys.argv[9:]:
    ...
    graph_count = command.count("--disable-cuda-graph")
    if graph_count != 1 - graphs:
        raise SystemExit(
...
proofs = (
    ...
    if spec and graphs
    else (("Capture target decode CUDA graph end",) if graphs else ())
)
for proof in proofs:
